  ...
  <div class="personal">
    <div class="personal-info">
      ...
      <el-button class="edit-info" round :icon="Edit" @click="goPage()"
        >Modify Personal Information</el-button
      >
    </div>
    ...
    <el-dialog v-model="dialogTableVisible" title="Change Profile Picture">
      <upload></upload>
    </el-dialog>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const collectSongList = ref([]); // Collected songs
    ...
    async function getCollection(userId) {
      ...
      const collectIDList = result.data || []; // Store collected song IDs
      // Fetch song information by song ID
      for (const item of collectIDList) {
        if (!item.songId) {
          console.error(`Song ${item} exception`);
          continue;
        }
        ...
        collectSongList.value.push(result.data);
      }
    }
