...
export default defineComponent({
  ...
  setup() {
    ...
    const lrcTop = ref("80px"); // Lyric scrolling
    const lyricArr = ref([]); // Lyrics of the current song
    const songId = computed(() => store.getters.songId); // Song ID
    const lyric = computed(() => store.getters.lyric); // Lyrics
    const currentPlayList = computed(() => store.getters.currentPlayList); // Stored music
    const currentPlayIndex = computed(() => store.getters.currentPlayIndex); // Position of the current song in the song list
    const curTime = computed(() => store.getters.curTime); // Current time
    const songTitle = computed(() => store.getters.songTitle); // Song title
    const singerName = computed(() => store.getters.singerName); // Singer's name
    const songPic = computed(() => store.getters.songPic); // Song image

    watch(songId, () => {
      lyricArr.value = parseLyric(currentPlayList.value[currentPlayIndex.value].lyric);
    });

    // Handle lyric position and color
    watch(curTime, () => {
